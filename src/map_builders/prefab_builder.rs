            ...
            for y in 0..layer.height {
                for x in 0..layer.width {
                    ...
                    if x < self.map.width as usize && y < self.map.height as usize {
                        ...
                        // we're doing some ugly nasty casting to make it easier to type things like '#' in the match
                        match (cell.ch as u8) as char {
                            ...
                            'M' => {
                                self.map.tiles[idx] = TileType::Floor;
                                self.spawns.push((idx, "Mopulon".to_string()));
                            }
                            'R' => {
                                self.map.tiles[idx] = TileType::Floor;
                                self.spawns.push((idx, "Recyculon".to_string()));
                            }
                            '^' => {
                                self.map.tiles[idx] = TileType::Floor;
                                self.spawns.push((idx, "Spike Trap".to_string()));
                            }
                            '%' => {
                                self.map.tiles[idx] = TileType::Floor;
