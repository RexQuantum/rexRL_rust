...
impl PrefabBuilder {
    ...
    fn char_to_map(&mut self, ch : char, idx: usize) {
        match ch {
            ...
            'M' => {
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Mopbot".to_string()));
            }
            'R' => {
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Recyculon".to_string()));
            }
            '^' => {
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Spike Trap".to_string()));
            }
            '%' => {
                self.map.tiles[idx] = TileType::Floor;
            ...
            '!' => {
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Repair Pack".to_string()));
            }
            _ => {
    ...
    fn apply_room_vaults(&mut self) {
        ...
        // Note to Rex: this is a placeholder and will be moved out of the function
        let master_vault_list = vec![TOTALLY_NOT_A_TRAP, CHECKERBOARD, SILLY_SMILE];

        // Filter the vault list down to vaults that are applicable to the current map/depth
