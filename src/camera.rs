...
fn get_tile_glyph(idx: usize, map : &Map) -> (rltk::FontCharType, RGB, RGB) {
    ...
    match map.tiles[idx] {
        ...
        TileType::WoodFloor => { glyph = rltk::to_cp437('='); fg = RGB::named(rltk::ROSY_BROWN); }
        TileType::Wall => {
            let x = idx as i32 % map.width;
            let y = idx as i32 / map.width;
            glyph = wall_glyph(&*map, x, y);
            fg = RGB::from_f32(0.2, 1.0, 1.0);
        }
        ...
        TileType::Road => { glyph = rltk::to_cp437('~'); fg = RGB::named(rltk::SANDY_BROWN); }
        ...
        TileType::Gravel => { glyph = rltk::to_cp437(';'); fg = RGB::named(rltk::GREEN_YELLOW);}
    }
